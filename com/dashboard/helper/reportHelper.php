...
class reportHelper {
	...
	function insertnewsupdate($databg=false, $datatq=false){ 
		...
		$sql = "INSERT INTO {$this->dbshemaWeb}.`tbl_template` (`userid`, `login_bg`, `thankyou_bg`, `login_type`, `redirect_url`, `submit_date`, `modified_date`, `n_status`)
				VALUES ('{$uid}', '{$databg}','{$datatq}','{$login_type}','{$redirecturi}', '{$submit_date}', '{$modified_date}', 1)";  
		// pr($sql);exit;
