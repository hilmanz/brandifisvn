...
class registerHelper {
				...
				// pr($sql);
				 $this->apps->query($sql);
					if($this->apps->getLastInsertId()>0)  {
							$this->findmyfrienddefault($qData['id']);
							// $this->createrolefriends($qData['id'],$otherid,$brandid,$brandsubid,$areaid);
						return 'Update Success.';
					}
				...
				// pr($sql);exit;
				 $this->apps->query($sql);
					if($this->apps->getLastInsertId()>0)  {
						...
						$this->findmyfrienddefault($lastID);
	...
	function userlists($orderBy='name',$orderType='ASC',$start=0,$limit=20,$search=null){
		...
		$sql  = "
			SELECT 
			sm.id,sm.name, sm.last_name ,sm.email,sm.username,sm.password,sm.birthday,sm.phone_number,sm.nickname,sm.sex,
