...
class contentHelper {
	...
	function __construct($apps){
		...
		$this->cocreationtypeid = "1,2";
		$this->brandtypeid = "1,3,4,5";
	...
	function getComment($cid=null,$all=false,$start=0,$limit=50,$summary=false,$pathlike=false){
		// return $cid;
		global $CONFIG;
	...
	function getArticleFeatured() {
		...
		$sql = "SELECT id,title,brief,image,thumbnail_image,slider_image,posted_date,file,url,fromwho,tags,authorid,topcontent,cityid ,articleType,can_save,content,categoryid
	...
	function getArticleContent($start=null,$limit=10,$contenttype=0,$topcontent=array(0,3),$articletype=false,$groupby=false,$author=false,$allcontent=false,$datetimes=false,$usingbadetail=true,$usingbranddetail=true,$checkin=true,$readarticle=false) {
		...
		if ($this->apps->_p('publishedtype')) {
			...
			$qPublishStatus = " AND anc.n_status =  {$publishedtype}  ";
		}else{
			// frontend
			$qPublishStatus = " AND anc.n_status =  1  ";
		}
		...
		if($articletype=='plan'){
			...
			$qCategories = "  AND pages.type IN (1,2,3,4,5) ";
			...
			// else $qStatus .= " AND MONTH(anc.posted_date) = MONTH(NOW()) ";
			if($brands=='cocreation') $qCategories = " AND pages.type IN (2) ";
			if($brands=='brands') $qCategories = " AND pages.type IN (3) ";
				...
				*/
					$qCheckCheckin = " 
						...
						AND NOT EXISTS ( 	SELECT id
							FROM {$this->dbshema}_news_content nc2
							WHERE nc2.parentid <> 0 AND anc.id = nc2.parentid 
						) 
						";
					$qSort = " abs(TIME_TO_SEC(anc.posted_date) - TIME_TO_SEC(NOW())) ASC, anc.id DESC ";
			}else{
				$qCheckCheckin = " AND  anc.parentid =0  ";
			}
			// pr($leadertype);
				if($leadertype==1){
					/* sba friend of sba shown */
					$qHirarki = " AND ( anc.authorid IN ({$uid}) OR EXISTS ( SELECT contentid FROM {$this->dbshema}_news_content_tags WHERE friendid IN ({$uid}) AND friendtype=1  AND contentid=anc.id AND anc.categoryid<>0 )) ";
		...
		if($typeid=='4'){
			 ...
			 $qSort = "  anc.topcontent DESC ,  anc.posted_date DESC   ";
		}
		
		if($allcontent == true )$qArticleType = "";
		else $qArticleType = " anc.articleType IN ({$typeid}) AND ";
		// if($typeid=='3') $qCheckCheckin = " OR EXISTS (SELECT contentid FROM my_checkin WHERE contentid=anc.id AND anc.n_status = 1 AND userid IN ({$uid}) ) OR  ( anc.parentid <> 0 AND anc.authorid IN ({$uid}) )  ";
		if($typeid=='3') $qCheckCheckin = " OR EXISTS (SELECT contentid FROM my_checkin WHERE contentid=anc.id 	{$qPublishStatus}   ) OR  ( anc.parentid <> 0 AND anc.authorid IN ({$uid}) )  ";
		...
		$sql = "
			SELECT 			anc.id,anc.title,anc.brief,anc.image,anc.thumbnail_image,anc.slider_image,anc.posted_date,anc.expired_date,anc.file,anc.url,anc.fromwho,anc.tags,anc.authorid,anc.topcontent,anc.cityid ,anc.articleType,anc.can_save,anc.n_status,pages.type pagetypes,anc.categoryid
			FROM {$this->dbshema}_news_content anc
			LEFT JOIN my_pages pages ON anc.authorid=pages.ownerid 
	...
	function getStatistictArticle($rqData=null){
		...
		foreach($rqData as $key => $val){
			...
			if(($val['categoryid']!=0)&&($val['articleType']==5)) $qData[$key]['plancheckincheck'] = true;
			else $qData[$key]['plancheckincheck'] = false;
		}		
		...
		$getCheckinByPlan = $this->getCheckinByPlan($cidStr);
		if($getCheckinByPlan){
			
			foreach($qData as $key => $val){
				if( $qData[$key]['plancheckincheck']) if(array_key_exists($val['id'],$getCheckinByPlan)) $qData[$key]['rating'] = $getCheckinByPlan[$val['id']];
		...
		//get friend tags new method
		$friendtagsData = $this->getFriendTaggedWithGalleryOnUser($cidStr,0,false);
		if($friendtagsData){
		...
		foreach($qData as $key => $val){				 
				...
				foreach($qData[$key]['gallery'] as $keys => $vale){
						$qData[$key]['gallery'][$keys]['friendtags'] = array();
						if($qData[$key]['friendtags']){							
							foreach($qData[$key]['friendtags'] as $vals){
								if($vale['id']==$vals['gid']) $qData[$key]['gallery'][$keys]['friendtags'][] = $vals;
							}
						}
				}
		}
		...
		foreach($qData as $key => $val){
			if($qData[$key]['friendtags']){
				foreach($qData[$key]['friendtags'] as $vals){
						$tempftags[$key]['friendtags'][$vals['id']] = $vals;
				}
			}else $tempftags[$key]['friendtags'] = array();
		}
		if($tempftags){
		
			foreach($qData as $key => $val){
				 $qData[$key]['friendtags'] = false;
					foreach($tempftags[$key]['friendtags'] as $vals){
							$qData[$key]['friendtags'][] = $vals;
					}
		...
		$friendtagsDataByPlan = $this->getFriendTaggedByPlan($cidStr);
		if($friendtagsDataByPlan){
			
			foreach($qData as $key => $val){
				if( $qData[$key]['plancheckincheck']) if(array_key_exists($val['id'],$friendtagsDataByPlan)) $qData[$key]['friendtags'] = $friendtagsDataByPlan[$val['id']];
				
			
			}
			// pr($friendtagsDataByPlan[29356]);exit;
		}
	...
	function getMygallery($start=null,$limit=3,$userid=NULL) {
		...
		$sql = "
			SELECT mm.*,nc.title,nc.brief,nc.image,nc.file,nc.articleType,ct.content as typecontent,ct.type as typeofarticle,nc.fromwho,nc.authorid,nc.posted_date , nc.url,nc.categoryid
			FROM `my_images` mm
	...
	function addUploadImage($data=false,$type=NULL){
		...
		if($categoryid==0) $n_statustags = 1;
		else  {
			...
			$isleadertype = @$this->apps->user->leaderdetail->type;
			$arrcount = 0;
			if($isleadertype==2) $arrcount = 1;
			if($isleadertype==3) $arrcount = 1;
			...
			// pr($arrcount);
			// exit;
				if(count($arrfidsnew)<=$arrcount){
					
					$pldetail = @$this->apps->user->pldetail;
					if($pldetail){
						foreach($pldetail as $val){
								$arrbadetail[$val->ownerid] = $val->ownerid;
						}
					}	
					
					$badetail = @$this->apps->user->badetail;
					if($isleadertype!=3){
						if($badetail){
							foreach($badetail as $val){
									$arrbadetail[$val->ownerid] = $val->ownerid;
							}
						}	
					}
					...
					WHERE sm.id IN ({$fids}) AND sm.n_status = 1 AND pages.type IN (1,2)
					
					";
					
					$qDatas = $this->apps->fetch($sql,1);	
					$fid = array();
					$ftype = array();
					foreach($qDatas as $val){
						$fid[$val['id']] = $val['id'];
						$ftype[$val['id']] = 1;
					}
					if($fid){
					...
					if(is_array($arrfid)){
						...
						if($frienddata){
							...
							foreach($frienddata as $val){
								if($val['fid']!=0){
								$this->addFriendTags($cid,$val['fid'],$val['ftype'],false,$message,$n_statustags);
						...
						$ftype = intval($ftype);
						if($fid!=0){
							$this->addFriendTags($cid,intval($fid),intval($ftype),false,$message,$n_statustags);						
	...
	function getFriendTagged($strCid=null,$tagin=0,$authorid=true){
		...
		WHERE contentid IN ({$strCid}) {$quserid} AND n_status IN(1) AND tagin = {$tagin} ";
		...
		$arrEntourageFid = false;
				foreach($qData as $val){	
				 // friendid 	friendtype 	date 	tagin 	n_status 
					...
					$gidtagged[$val['contentid']][$val['friendtype']][$val['friendid']]= $val['contentid'];
					if($val['gid']!=0) $gidtagged[$val['contentid']][$val['friendtype']][$val['friendid']]= $val['gid'];
					// pr($val['gid']);
				}
				// pr($gidtagged);exit;
				$socialdata = false;
				...
				foreach($frienddata as $contentkey => $ftypeval){
					foreach($ftypeval as $keyftype => $ftype){
						...
						//gallery tagged revision
						if($socialdata)if(array_key_exists($keyftype,$socialdata)) if(array_key_exists($key,$socialdata[$keyftype])) $socialdata[$keyftype][$key]['gid'] = $gidtagged[$val['contentid']][$keyftype][$key];
						if($entouragedata) if(array_key_exists($keyftype,$entouragedata)) if(array_key_exists($key,$entouragedata[$keyftype]))   $entouragedata[$keyftype][$key]['gid'] = $gidtagged[$val['contentid']][$keyftype][$key];
	...
	function getFriendTaggedWithGalleryOnUser($strCid=null,$tagin=0,$authorid=true){
		if(!$this->uid) return false;
		if($strCid==null) return false;
		if($authorid)$quserid = " AND userid={$this->uid} ";
		else $quserid = "";
		...
		$arrEntourageFid = false;
				foreach($qData as $val){	
					// checkin user datas not mapping
					/* BA */	
					if($val['friendtype']==0) $arrEntourageFid[$val['friendid']] = $val['friendid'];
					/* entourage */
					if($val['friendtype']==1) $arrSocialFid[$val['friendid']] = $val['friendid'];
					...
					// mapping user data with gid and contentid
					$frienddata[$val['contentid']][$val['gid']][$val['friendtype']][$val['friendid']]= $val;
					$gidtagged[$val['contentid']][$val['gid']][$val['friendtype']][$val['friendid']]= $val['contentid'];
					if($val['gid']!=0) $gidtagged[$val['contentid']][$val['gid']][$val['friendtype']][$val['friendid']]= $val['gid'];
					// pr($val['gid']);
				}
				// pr($gidtagged);exit;
				$socialdata = false;
				...
				//merge data
				foreach($frienddata as $contentkey => $gidval){
					foreach($gidval as $gidkey => $ftypeval){
						foreach($ftypeval as $keyftype => $ftype){
							foreach($ftype as $key => $val){
							//gallery tagged revision
							if($socialdata)if(array_key_exists($keyftype,$socialdata)) if(array_key_exists($key,$socialdata[$keyftype])) $socialdata[$keyftype][$key]['gid'] = $gidtagged[$val['contentid']][$gidkey][$keyftype][$key];
							if($entouragedata) if(array_key_exists($keyftype,$entouragedata)) if(array_key_exists($key,$entouragedata[$keyftype]))   $entouragedata[$keyftype][$key]['gid'] = $gidtagged[$val['contentid']][$gidkey][$keyftype][$key];
	...
	function getFriendTaggedByPlan($strCid=null,$tagin=0,$authorid=true){
		if(!$this->uid) return false;
		if($strCid==null) return false;
		if($authorid)$quserid = " AND userid={$this->uid} ";
		else $quserid = "";
		
		$sql = "SELECT id,parentid FROM beat_news_content WHERE parentid IN ({$strCid}) AND articleType = 5 AND categoryid<> 0 AND authorid={$this->uid} ";
		$qDataPlan = $this->apps->fetch($sql,1);
		// pr($sql);
		if($qDataPlan){
			$arrparentidcontent = false;
			$arridcontent = false;
			foreach($qDataPlan as $val){
				$arrparentidcontent[$val['id']] = $val['parentid'];
				$arridcontent[$val['id']] = $val['id'];
			}
			if($arrparentidcontent&&$arridcontent){
					$strCid = implode(',',$arridcontent);
			}else return false;
		}else return false;
		...
		// pr($sql);
		$this->logger->log("tag oin plan ".$sql);
		$qData = $this->apps->fetch($sql,1);
		if(!$qData) return false;
		$data = false;
		$arrSocialFid = false;
		$arrEntourageFid = false;
				...
				foreach($frienddata as $contentkey => $ftypeval){
					foreach($ftypeval as $keyftype => $ftype){
						...
						if($socialdata)if(array_key_exists($keyftype,$socialdata)) if(array_key_exists($key,$socialdata[$keyftype]))  $data[$arrparentidcontent[$val['contentid']]][] = $socialdata[$keyftype][$key];
						
						if($entouragedata) if(array_key_exists($keyftype,$entouragedata)) if(array_key_exists($key,$entouragedata[$keyftype]))  $data[$arrparentidcontent[$val['contentid']]][] = $entouragedata[$keyftype][$key];
						 // pr($val);
						}
	...
	function addFriendTags($cid=0,$fid=0,$ftype='pass',$gallery=false,$message=false,$n_status=1,$gid=0){
		
		if(!$this->uid) return false;
		if( $cid==0 ) $cid = intval($this->apps->_p("cid"));
		if( $fid==0 ) $fid = intval($this->apps->_p("fid"));
		// $this->logger->log("before sending data : ".$ftype);
		if( ((string)$ftype)=='pass') {
			// $this->logger->log("must be not here after sending data cos : ".$ftype);
			$ftype = intval($this->apps->_p("ftype"));
		}
			// $this->logger->log("after sending data : ".$ftype);
		...
		$sql=" 
				INSERT INTO 
				{$this->dbshema}_news_content_tags 	(userid ,	contentid 	,friendid, 	friendtype, date ,	n_status, tagin,gid ) 
				VALUES ({$this->uid},{$cid},{$fid},'{$ftype}',NOW(),'{$n_status}',{$gallery},{$gid})
				ON DUPLICATE KEY UPDATE n_status = 1, date=NOW(),gid=VALUES(gid)
				";
			// pr($sql);exit;
			$this->logger->log($sql);
	...
	function getMyFavorite($userid=0,$limit=10){
			...
			$sql ="
				...
				if($qData) {
					...
					//get content
					$sql = "
					SELECT anc.id,anc.title,anc.brief,anc.image,anc.thumbnail_image,anc.slider_image,anc.posted_date,anc.file,anc.url,anc.fromwho,anc.tags,anc.authorid,anc.topcontent,anc.cityid,anct.type pagesname,anc.categoryid FROM athreesix_news_content anc
					LEFT JOIN {$this->dbshema}_news_content_type anct ON anct.id = anc.articleType					
	...
	function getMyCalendar($userid=0,$mypagestype=0,$limit=10){
			...
			if($mypagestype>=2) $qFromWho = " AND anc.fromwho = {$mypagestype} ";
				...
				// pr($sql);
					...
					//get content
					$sql = "
					SELECT anc.id,anc.title,anc.brief,anc.image,anc.thumbnail_image,anc.slider_image,anc.posted_date,anc.file,anc.url,anc.fromwho,anc.tags,anc.authorid,anc.topcontent,anc.cityid,anct.type pagesname,anc.categoryid FROM {$this->dbshema}_news_content anc
					LEFT JOIN {$this->dbshema}_news_content_type anct ON anct.id = anc.articleType					
	...
	function unContentPost(){
		...
		$notqEditPlan = "";
		if($type==5){
			$notqEditPlan = " AND posted_date <= DATE_SUB(NOW(),INTERVAL 30 days) ";
		}
		
		$sql = "UPDATE {$this->dbshema}_news_content SET n_status = {$publishedtype} WHERE id = '{$cid}' AND fromwho=1 {$qauthorid} LIMIT 1";
		$this->logger->log(" delete content : ".$sql);
			// pr($sql);exit; AND articleType={$type} 
		if ($this->apps->query($sql)) {
			return true;
		} else return false;
		return false;
	...
	function addUploadImageGallery($data=false,$type=NULL){
		...
		if(@$this->apps->user->leaderdetail->type==1) if(!$this->capsgalleryuploader($cid,48)) return false;
		...
		$sql ="
			...
			// pr($sql);exit;
			$this->logger->log($sql);
			$this->logger->log(" fid :".$friendtags);
			$this->logger->log(" ftype :".$friendtypetags);
			$this->apps->query($sql);
			if($this->apps->getLastInsertId()>0){
				$message = " {$this->apps->user->name} {$this->apps->user->last_name} {$LOCALE[1]['tagged']} {$title} ";
				...
				if($friendtags){					
					...
					if(is_array($arrfid)){
						foreach($arrfid as $key => $val){
							$frienddata[$key]['fid'] = $val;
								//temporary fixing
							//	if($val>250) $frienddata[$key]['ftype'] = 0;
							//	else  $frienddata[$key]['ftype'] = 1;
						...
						if($frienddata){
							...
							foreach($frienddata as $val){
								$this->addFriendTags($cid,$val['fid'],$val['ftype'],false,$message,1,$galleryid);
							}
						
						}
					}else{
						$friendtypetags = intval($friendtypetags);
						$this->addFriendTags($cid,intval($friendtags),intval($friendtypetags),false,$message,1,$galleryid);
					}
				}
	...
	function capsgalleryuploader($cid=false,$days=48){
		if($cid==false) return true;
		
		$sql ="  
			SELECT posted_date,id 
			FROM {$this->dbshema}_news_content 
			WHERE 
				id={$cid}  
				AND posted_date <= NOW()
				AND DATE_ADD(posted_date,INTERVAL {$days} DAY) >= NOW()
		";
	...
	function editContentArticle($data=false){
		...
		$recipientid = $this->apps->_p('recipientid');
		if(!$recipientid)$fid = $this->apps->_p('fid');
		else $fid = $this->apps->_p('recipientid');
	...
	function getCheckinByPlan($cidstr=false){
		if($cidstr==false) return false;
		
		$sql = "SELECT id,parentid FROM beat_news_content WHERE parentid IN ({$cidstr}) AND articleType = 5 AND categoryid<> 0 AND authorid={$this->uid} ";
		$qDataPlan = $this->apps->fetch($sql,1);
		// pr($sql);
		if($qDataPlan){
			$arrparentidcontent = false;
		...
		foreach($qData as $key => $val){	
			...
			$dataceckin[$arrparentidcontent[$val['contentid']]] = $qData[$key];	
	...
	function getGalleryTypeContent($start=null,$limit=12) {
		...
		$brands = strip_tags($this->apps->_request('category'));
		$qCategories = " ";
		if($brands=='cocreation') $qCategories = " AND pages.type IN ({$this->cocreationtypeid}) AND  anc.articleType = 5 AND anc.categoryid = 2  ";
		if($brands=='brands') $qCategories = " AND pages.type IN ({$this->brandtypeid}) AND  anc.articleType = 5 AND anc.categoryid = 3   ";
		if($brands=='timeline') {
		...
		if($brands=='myplan') {
			$qCategories = " AND pages.type IN ( 1 ) AND anc.articleType = 5 AND anc.categoryid = 0  ";
				$qHirarki = " AND ( authorid IN ({$uid}) OR EXISTS ( SELECT contentid FROM {$this->dbshema}_news_content_tags WHERE friendid IN ({$uid}) AND friendtype=1  AND contentid=anc.id) ) ";
		}
	...
	function getMerchandiseList($start=null,$limit=12){
		...
		$limit = intval($limit);
		$m_type = '0,4,5';
		if(intval($this->apps->_p('m_type'))=='0'){
			$m_type=0;
		}else{
	...
	function getChallangeData($cid=false){
		...
		if($qData['imagepath']) $qData['image_full_path'] = $CONFIG['BASE_DOMAIN_PATH']."public_assets/".$qData['imagepath']."/".$qData['image'];
