...
class newsHelper{
	...
	function sendGlobalMail($to,$from,$msg,$subject=false,$bugreport=false,$attachment=false){
		...
		if(!$from)$mail->SetFrom($CONFIG['EMAIL_FROM_DEFAULT'], 'Facemaker App');
		else $mail->SetFrom($from['from'], $from['alias']);

		if(!$subject) $mail->Subject    = " Face of a decision maker ";
		else $mail->Subject = $subject;
		
		$mail->AltBody    = "To view the message, please use an HTML compatible email viewer!"; // optional, comment out and test

		if(!$bugreport) $mail->MsgHTML($msg);
		else $mail->MsgHTML($msg);
		...
		// pr($attachment);
		if($attachment) {
			// pr($attachment['filerealpath']);
			$mail->AddAttachment($attachment['filerealpath']);      // attachment
		}
