...
class registerHelper {
	...
	//pr($sql);exit;
		...
		//GET LIST
		$sql = "
			SELECT *,sm.name as nama 
			FROM {$CONFIG['DATABASE'][0]['DATABASE']}.social_member  sm
			LEFT JOIN {$CONFIG['DATABASE'][0]['DATABASE']}.my_profile mp ON sm.id=mp.ownerid 
			LEFT JOIN {$CONFIG['DATABASE'][0]['DATABASE']}.city_reference  cr ON mp.city=cr.id 
			LEFT JOIN {$CONFIG['DATABASE'][0]['DATABASE']}.tbl_brand_master bm ON mp.brand=bm.id 
			LEFT JOIN {$CONFIG['DATABASE'][0]['DATABASE']}.my_profile_type pt ON mp.type=pt.id 
	...
	function insertnewdata(){
		...
		if ($password==!NULL&&$name==!NULL)
		{
			if ($password==$repassword)
			{
				...
				if($submit){
					$sql1 = "INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.social_member (`name`, `nickname`,`email`,`username`,`sex`,`last_name`,`salt`,`password`,n_status) 
							VALUES ('{$name}', '{$nickname}', '{$email}','{$username}','{$gender}','{$last_name}','{$saltnya}','{$hash}',1)";
					...
					if($lastID>0)  { 
						$sql2 = "INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.my_profile (`name`, `created_date`,`closed_date`,`type`,`brand`,`city`,ownerid,n_status) 
							VALUES ('{$name}', '{$created_date}', '{$closed_date}', '{$role}', '{$o_brand}', '{$city}', '{$lastID}',1)";
